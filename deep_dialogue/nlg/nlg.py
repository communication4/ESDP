...
class nlg:
    ...
    def convert_diaact_to_nl(self, dia_act, turn_msg):
        ...
        if dia_act['diaact'] == 'inform' and 'taskcomplete' in list(dia_act['inform_slots'].keys()) and \
    ...
    def load_nlg_model(self, model_path):
        ...
        model_params = Tools().openPickleFileBy3(model_path)
    ...
    def diaact_to_nl_slot_filling(self, dia_act, template_sentence):
        ...
        counter = 0
        # print("dia_act['inform_slots'].keys()",list(dia_act['inform_slots'].keys()))
        for slot in list(dia_act['inform_slots'].keys()):
