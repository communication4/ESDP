...
class DialogManager:
    ...
    def initialize_episode(self, use_environment=False, goal_idx=None):
        ...
        self.user_action = self.running_user.initialize_episode(goal_idx)
    ...
    def next_turn(self, record_training_data=True, record_training_data_for_user=True):
        ...
        estate_t = self.running_user.state['emotion']
        # print 'user emotion state, estate_t::', estate_t
        self.agent_action = self.agent.state_to_action(self.state, estate_t)

        with open("record.txt", mode='a', encoding='utf-8') as file_obj:
            file_obj.write("User:")
            file_obj.write("\n")
            file_obj.write(str(self.state["user_action"]))
            file_obj.write("\n")
            file_obj.write("Agent:")
            file_obj.write("\n")
            file_obj.write(str(self.agent_action))
            file_obj.write("\n")
            file_obj.close()
        ...
        if self.use_world_model:
            ...
            self.dialogue_reward = self.reward_function(dialog_status)
            self.emotion_reward = self.emotional_reward_function(dialog_status, self.running_user.state)
            self.reward = self.dialogue_reward # + self.emotion_reward


        # print("AGENT:", self.agent_action)
        ...
        estate_tplus1 = self.running_user.state['emotion']
        ...
        if record_training_data:
            self.agent.register_experience_replay_tuple(self.state, estate_t, self.agent_action, self.reward, self.emotion_reward, 
                                                        self.state_tracker.get_state_for_agent(), estate_tplus1, self.episode_over,
        ...
        return (self.episode_over, self.reward, self.dialogue_reward, self.emotion_reward)
    ...
    def emotional_reward_function(self, dialog_status, user_status):
        """ Reward Function 1: a reward function based on the dialog_status """
        if True:
            em_cur_reward = user_status['emotion']
            em_prev_reward = user_status['historical_emotion'][-1]
            emotional_reward = -1.0 * ((em_cur_reward[1]-em_prev_reward[1]) + (em_cur_reward[2]-em_prev_reward[2]) + (em_cur_reward[4]-em_prev_reward[4]) - (em_cur_reward[5]-em_prev_reward[5]) - (em_cur_reward[6]-em_prev_reward[6]))
