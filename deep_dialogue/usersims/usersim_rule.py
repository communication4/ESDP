...
from emosim import EmoSimulator
from eventdet import DstcEventDetector
from emo_configs import EmoConfig
from user_profiles import ProfileReader
...
class RuleSimulator(UserSimulator):
    ...
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, start_set=None, params=None):
        ...
        self.emo_configs = EmoConfig('./emo_configs/emosim_configs.json')
        self.profile_reader = ProfileReader('./user_profiles/profiles.json')
        self.user_profiles = self.profile_reader.get_profile()

        # configs = {'emo_calculator': 'LinearEmoCalculator', 'alpha':1.0, 
        #         'decay_factor':[-0.01, -0.01, -0.01, -0.1], 'scale_factor':8.0,
        #         'dataset':'dstc', 'event_detector':'RuleEventDetector',
        #     'turn_threshold':25}
        # profile = {'mood':[0.0, 0.0, 0.0, 0.0], 'personality':[0.8, 0.1, 0.87, 0.5, 0.05]}

        self.emo_simulator = EmoSimulator(self.emo_configs.get_configs(), self.user_profiles[0])
        self.event_detector = DstcEventDetector(self.emo_configs.get_configs())
    ...
    def initialize_episode(self, goal_idx=None):
        ...
        self.emo_simulator.set_mood([0.0, 0.0, 0.0, 0.0])
        self.state['emotion'] = self.emo_simulator.get_mood()
    ...
    def update_user_emotion(self, sys_act):

        self.state['historical_emotion'].append(self.state['emotion'])

        events = self.event_detector.detect(self.state, sys_act, self.goal).counts
        # print(events)
        # print("Before:", self.emo_simulator.get_mood())

        self.state['trigger_counter'][0] += events[0]
        self.state['trigger_counter'][1] += events[1]
        self.state['trigger_counter'][2] += events[2]
        self.state['trigger_counter'][3] += events[3]
        self.state['trigger_counter'][4] += events[4]

        self.emo_simulator.update_mood(events)
        # print("After:", self.emo_simulator.get_mood())
    ...
    def  next(self, system_action):
        ...
        self.update_user_emotion(system_action)
    ...
    def response_request(self, system_action):
        ...
        if len(list(system_action['request_slots'].keys())) > 0:
            ...
            if slot in list(self.goal['inform_slots'].keys()): # request slot in user's constraints  #and slot not in self.state['request_slots'].keys():
                ...
                self.state['request_slots'].clear()
            elif slot in list(self.goal['request_slots'].keys()) and slot not in self.state['rest_slots'] and slot in self.state['history_slots'].keys(): # the requested slot has been answered
