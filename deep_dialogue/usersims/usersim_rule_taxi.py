...
class RuleTaxiSimulator(UserSimulator):
    ...
    def __init__(self, taxi_dict=None, act_set=None, slot_set=None, start_set=None, personality_set=None, emotion_set=None, params=None):
        """ Constructor shared by all user simulators """

        super(RuleTaxiSimulator, self).__init__()
    ...
    def initialize_episode(self, goal_idx=None):
        ...
        self.goal['request_slots']['Ref'] = 'UNK'
    ...
    def _sample_action(self):
        ...
        if len(self.goal['inform_slots']) > 0:
            ...
            if 'Depart' in self.goal['inform_slots'].keys(): # 'taxiname' must appear in the first user turn
                self.state['inform_slots']['Depart'] = self.goal['inform_slots']['Depart']
                
            # if 'greeting' in self.goal['inform_slots'].keys():
            #     self.state['inform_slots']['greeting'] = self.goal['inform_slots']['greeting']
                
            for slot in self.goal['inform_slots'].keys():
                if known_slot == slot or slot == 'Depart' or slot == 'greeting': continue
    ...
    def debug_falk_goal(self):
        ...
        self.goal['inform_slots']['People'] = '4'
        ...
        self.goal['inform_slots']['Depart'] = 'cocum'
        self.goal['inform_slots']['Area'] = 'west'
        self.goal['inform_slots']['Arrive'] = '04:15'
        #self.goal['inform_slots']['state'] = 'nc'
        
        self.goal['request_slots'].clear()
        self.goal['request_slots']['Ref'] = 'UNK'
        self.goal['request_slots']['Car'] = 'UNK'
        self.goal['request_slots']['Leave'] = 'UNK'
        self.goal['request_slots']['Dest'] = 'UNK'
        #self.goal['request_slots']['cost'] = 'UNK'
        self.goal['request_slots']['day'] = 'UNK'
        ...
        for info_slot in self.state['history_slots'].keys():
            ...
            if info_slot in self.goal['inform_slots'].keys():
                if self.state['history_slots'][info_slot] != self.goal['inform_slots'][info_slot]:
                    self.dialog_status = dialog_config.FAILED_DIALOG
                    print("FAILED Because THE INFORMATION ARE CHANGED, slot is %s but goal is %s" % (self.state['history_slots'][info_slot], self.goal['inform_slots'][info_slot]) )

        if 'Ref' in system_action['inform_slots'].keys():
            if system_action['inform_slots']['Ref'] == dialog_config.NO_VALUE_MATCH:
                self.dialog_status = dialog_config.FAILED_DIALOG
    ...
    def response_inform(self, system_action):
        ...
        if 'taskcomplete' in system_action['inform_slots'].keys(): # check all the constraints from agents with user goal
            ...
            if system_action['inform_slots']['taskcomplete'] == dialog_config.NO_VALUE_MATCH:
                self.state['history_slots']['Ref'] = dialog_config.NO_VALUE_MATCH
                if 'Ref' in self.state['rest_slots']: self.state['rest_slots'].remove('Ref')
                if 'Ref' in self.state['request_slots'].keys(): del self.state['request_slots']['Ref']
            ...
            for slot in system_action['inform_slots'].keys():
                ...
                if slot in self.goal['inform_slots'].keys():
                    if system_action['inform_slots'][slot] == self.goal['inform_slots'][slot]:
                        ...
                        if len(self.state['request_slots']) > 0:
                            ...
                            if len(rest_slot_set) > 0:
                                ...
                                self.state['request_slots']['Ref'] = 'UNK'
                                self.state['diaact'] = "request"
                    ...
                    if len(self.state['request_slots']) > 0:
                        ...
                        if 'Ref' in request_set:
                            request_set.remove('Ref')
