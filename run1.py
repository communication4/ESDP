...
from deep_dialog.dialog_system import DialogManager, text_to_dict, json_to_dict
from deep_dialog.agents import AgentCmd, InformAgent, RequestAllAgent, RandomAgent, EchoAgent, RequestBasicsAgent, AgentDQN
from deep_dialog.usersims import RuleSimulator, ModelBasedSimulator,RuleTaxiSimulator, RuleRestSimulator
...
def load_actions(sys_req_slots, sys_inf_slots, usr_req_slots, usr_inf_slots):
    ...
    dialog_config.feasible_actions_users = [
        {'diaact': "thanks", 'inform_slots': {}, 'request_slots': {}},
        {'diaact': "deny", 'inform_slots': {}, 'request_slots': {}},
    ...
    for slot in usr_inf_slots:
        if slot != 'greeting':
            dialog_config.feasible_actions_users.append(
                {'diaact': 'inform', 'inform_slots': {slot: "PLACEHOLDER"}, 'request_slots': {}}
            )

    for slot in usr_req_slots:
        if slot != 'greeting':
            dialog_config.feasible_actions_users.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dict_path', dest='dict_path', type=str, default='./deep_dialog/data_taxi1/slot_dict.v1_.json',
                        help='path to the .json dictionary file')
    parser.add_argument('--movie_kb_path', dest='movie_kb_path', type=str, default='./deep_dialog/data_taxi1/taxi.kb.2k.v1_.json',
                        help='path to the movie kb .json file')
    parser.add_argument('--act_set', dest='act_set', type=str, default='./deep_dialog/data_taxi1/dia_acts.txt',
                        help='path to dia act set; none for loading from labeled file')
    parser.add_argument('--slot_set', dest='slot_set', type=str, default='./deep_dialog/data_taxi1/taxi_slots.txt',
                        help='path to slot set; none for loading from labeled file')
    parser.add_argument('--goal_file_path', dest='goal_file_path', type=str,
                        default='./deep_dialog/data_taxi1/user_goals_first.part.taxi.v4_.json',
    ...
    parser.add_argument('--episodes', dest='episodes', default=300, type=int,
    ...
    parser.add_argument('--agt', dest='agt', default=13, type=int,
                        help='Select an agent: 0 for a command line input, 1-6 for rule based agents')
    parser.add_argument('--usr', dest='usr', default=2, type=int,
    ...
    parser.add_argument('--gamma', dest='gamma', type=float, default=0.95, help='gamma for DQN')
    ...
    parser.add_argument('--warm_start_epochs', dest='warm_start_epochs', type=int, default=200,
                        help='the number of epochs for warm start')
    parser.add_argument('--planning_steps', dest='planning_steps', type=int, default=0,
    ...
    parser.add_argument('-o', '--write_model_dir', dest='write_model_dir', type=str,
                        default='./deep_dialog/checkpoints/taxi_esdp_07_20/', help='write model to disk')
    ...
    parser.add_argument('--boosted', dest='boosted', type=int, default=0, help='Boost the world model')
    parser.add_argument('--train_world_model', dest='train_world_model', type=int, default=0,
    ...
    parser.add_argument('--personality_set', dest='personality_set', type=str, default='./deep_dialog/data/personality_set.json',
                        help='a list of personalities')
    parser.add_argument('--emotion_set', dest='emotion_set', type=str, default='./deep_dialog/data/emotion_set.json',
                        help='a list of emotions')
    parser.add_argument('--user_abort', dest='user_abort', type=int, default=0,
                        help='user abort')
    parser.add_argument('--abort_threhold', dest='abort_threhold', type=float, default=0.5,
                        help='abort threhold')
    parser.add_argument('--abort_rate', dest='abort_rate', type=float, default=0.01,
                        help='abort rate')

    parser.add_argument('--alpha', dest='alpha', type=float, default=0.9)
    parser.add_argument('--beta', dest='beta', type=float, default=20)
...
with open(goal_file_path, 'r') as f1:
    all_goal_set = json.load(f1)
...
with open(movie_kb_path, 'r') as f2:
    movie_kb = json.load(f2)

act_set = text_to_dict(params['act_set'])
slot_set = text_to_dict(params['slot_set'])
personality_set = json_to_dict(params['personality_set'])
emotion_set = json_to_dict(params['emotion_set'])

################################################################################
# a movie dictionary for user simulator - slot:possible values
################################################################################
#movie_dictionary = pickle.load(open(dict_path, 'rb'))
with open(dict_path, 'r') as f3:
    movie_dictionary = json.load(f3)
...
agent_params['alpha'] = params['alpha']
agent_params['beta'] = params['beta']
agent_params['topk'] = params['topk']
...
if agt == 0:
    ...
    load_actions(dialog_config.taxi_sys_request_slots, dialog_config.taxi_sys_inform_slots,
                 dialog_config.taxi_user_request_slots, dialog_config.taxi_user_inform_slots)
    agent = AgentDQN(movie_kb, act_set, slot_set, agent_params)
    ...
    load_actions(dialog_config.restaurant_sys_request_slots, dialog_config.restaurant_sys_inform_slots,
                 dialog_config.restaurant_user_request_slots, dialog_config.restaurant_user_inform_slots)
    agent = AgentDQN(movie_kb, act_set, slot_set, agent_params)
...
usersim_params['user_abort'] = params['user_abort']
usersim_params['abort_threhold'] = params['abort_threhold']
usersim_params['abort_rate'] = params['abort_rate']

if usr == 0:  # real user
    # user_sim = RealUser(movie_dictionary, act_set, slot_set, goal_set, usersim_params)
    pass
elif usr == 1:
    user_sim = RuleSimulator(movie_dict=movie_dictionary, act_set=act_set, slot_set=slot_set, start_set=goal_set, params=usersim_params)
    ...
    user_sim = RuleTaxiSimulator(movie_dictionary, act_set, slot_set, goal_set,
                                 personality_set=personality_set, emotion_set=emotion_set, params=usersim_params)
    world_model = ModelBasedSimulator(movie_dictionary, act_set, slot_set, goal_set, usersim_params)
    agent.set_user_planning(world_model)
elif usr == 3:
    user_sim = RuleRestSimulator(movie_dictionary, act_set, slot_set, goal_set,
                                 personality_set=personality_set, emotion_set=emotion_set, params=usersim_params)
    world_model = ModelBasedSimulator(movie_dictionary, act_set, slot_set, goal_set, usersim_params)
    agent.set_user_planning(world_model)
# elif usr == 2:
...
performance_records['ave_dialogue_reward'] = {}
performance_records['ave_emotional_reward'] = {}
performance_records['ave_emo'] = {}
performance_records['ave_hit'] = {}
performance_records['success_labels'] = {}
...
def simulation_epoch(simulation_epoch_size):
    ...
    cumulative_reward = 0
    cumulative_turns = 0
    cumulative_dialogue_reward = 0
    cumulative_emotional_reward = 0
    success_labels = []
    cumulative_emotions = []
    cumulative_counters = []
    ...
    for episode in range(simulation_epoch_size):
        dialog_manager.initialize_episode(use_environment=True, goal_idx=episode)
        episode_over = False
        while (not episode_over):
            episode_over, reward, d_reward, e_reward = dialog_manager.next_turn(record_training_data_for_user=False)
            cumulative_reward += reward
            cumulative_dialogue_reward += d_reward
            cumulative_emotional_reward += e_reward
            if episode_over:
                if reward > 0:
                    successes += 1
                    print ("simulation episode %s: Success" % (episode))
                    success_labels.append(1)
                    # print ("simulation episode %s: Success, with emotion distribution: %s" % (episode, str(dialog_manager.user.state['emotion'])))
                else:
                    print ("simulation episode %s: Fail" % (episode))
                    success_labels.append(0)
                    # print ("simulation episode %s: Fail, with emotion distribution: %s" % (episode, str(dialog_manager.user.state['emotion'])))
                cumulative_turns += dialog_manager.state_tracker.turn_count
                cumulative_emotions.append(dialog_manager.user.state['emotion'])
                cumulative_counters.append(dialog_manager.user.state['trigger_counter'])

    res['success_rate'] = float(successes) / simulation_epoch_size
    res['ave_reward'] = float(cumulative_reward) / simulation_epoch_size
    res['ave_turns'] = float(cumulative_turns) / simulation_epoch_size
    res['ave_dialogue_reward'] = float(cumulative_dialogue_reward) / simulation_epoch_size
    res['ave_emotional_reward'] = float(cumulative_emotional_reward) / simulation_epoch_size
    res['ave_emo'] = np.mean(cumulative_emotions, axis=0).tolist()
    res['ave_hit'] = np.mean(cumulative_counters, axis=0).tolist()
    res['total_hit'] = np.sum(cumulative_counters, axis=0).tolist()
    res['success_labels'] = success_labels
    print ("simulation success rate %s, ave reward %s, ave dialogue reward %s, ave emotional reward %s, ave turns %s" % (
        res['success_rate'], res['ave_reward'], res['ave_dialogue_reward'], res['ave_emotional_reward'], res['ave_turns']))
    return res
...
def simulation_epoch_for_training(simulation_epoch_size, grounded_for_model=False):
    ...
    for episode in range(simulation_epoch_size):
        ...
        episode_over = False
        while (not episode_over):
            episode_over, reward, d_reward, e_reward = dialog_manager.next_turn()
            cumulative_reward += reward
            cumulative_dialogue_reward += d_reward
            cumulative_emotional_reward += e_reward
            if episode_over:
...
def warm_start_simulation():
    ...
    for episode in range(warm_start_epochs):
        with open("record.txt", mode='a', encoding='utf-8') as file_obj:
            file_obj.write("episode:{} \n".format(episode))
            file_obj.close()
...
def run_episodes(count, status):
    ...
    with open(file_path, mode='w', encoding='utf-8') as file_obj:
        file_obj.write('dialogue record \n')
        file_obj.close()

    if (agt == 9 or agt == 13 or agt ==14) and params['trained_model_path'] == None and warm_start == 1:
    ...
    for episode in range(count):
        ...
        while not episode_over:
            ...
            episode_over, reward, _, _ = dialog_manager.next_turn(record_training_data_for_user=False)
            cumulative_reward += reward
            ...
            if episode_over:
                ...
                cumulative_turns += dialog_manager.state_tracker.turn_count
        end_time = time.time()
        cost = (end_time - start_time)/turn * 10
        all_cost.append(cost)
        #print("the cost is" + str(cost))
        # simulation
        if (agt == 9 or agt == 13 or agt ==14) and params['trained_model_path'] == None:
            ...
            simulation_res = simulation_epoch(len(goal_set[params['learning_phase']]))

            performance_records['success_rate'][episode] = simulation_res['success_rate']
            performance_records['ave_turns'][episode] = simulation_res['ave_turns']
            performance_records['ave_reward'][episode] = simulation_res['ave_reward']
            performance_records['ave_emo'][episode] = simulation_res['ave_emo']
            performance_records['ave_hit'][episode] = simulation_res['ave_hit']
            performance_records['ave_dialogue_reward'][episode] = simulation_res['ave_dialogue_reward']
            performance_records['ave_emotional_reward'][episode] = simulation_res['ave_emotional_reward']
            performance_records['success_labels'][episode] = simulation_res['success_labels']
            # performance_records['total_hit'][episode] = simulation_res['total_hit']
            ...
            agent.train_cls(batch_size, 1)
            agent.train_dqn(batch_size, 1)
